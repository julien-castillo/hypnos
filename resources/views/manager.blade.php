            ...
            <table class="table table-bordered table-hover">
                ...
                @foreach($suites as $suite)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('manager.suite.edit', ['suite' => $suite->id]) }}"
                               class="btn btn-warning">Editer</a>
                            <a href="#" class="btn btn-danger"
                               onclick="if(confirm('Voulez-vous vraiment supprimer cette suite ?')){document.getElementById('form-{{$suite->id}}').submit() }">Supprimer</a>
                            <form id="form-{{ $suite->id }}"
                                  action="{{ route('manager.suite.delete', ['suite'=>$suite->id]) }}" method="post">
                                @csrf
